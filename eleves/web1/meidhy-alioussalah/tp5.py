...
# Interdit : count()
resultat_ex5 = 0
for c in donnees["chaine1"]:
    if c == donnees["lettre1"]:
        resultat_ex5 += 1
...
# Interdit : sum()
resultat_ex6 = resultat_ex6 = 0
for i in range(2, donnees["somme_fin"] + 1, 2):
    resultat_ex6 += i
...
resultat_ex8 = resultat_ex8 = {}
for i in range(len(donnees["chaine2"])):
    resultat_ex8[i] = donnees["chaine2"][i]
...
resultat_ex9 = -1
trouve = False
for i in range(len(donnees["chaine3"])):
    if not trouve and donnees["chaine3"][i] == donnees["lettre2"]:
        resultat_ex9 = i
        trouve = True
...
# Interdit : [::-1]
resultat_ex10 = ""
for i in range(len(donnees["mot1"]) - 1, -1, -1):
    resultat_ex10 += donnees["mot1"][i]
...
resultat_ex11 = ""
for c in donnees["chaine4"]:
    resultat_ex11 += chr(ord(c) - 32)
...
resultat_ex13 = []
for i in range(1, 11):
    resultat_ex13.append(str(donnees["table_n"]) + " x " + str(i) + " = " + str(donnees["table_n"] * i))
...
resultat_ex14 = []
for ligne in range(donnees["carre_n"]):
    chaine = ""
    for colonne in range(donnees["carre_n"]):
        chaine += "*"
    resultat_ex14.append(chaine)
...
resultat_ex15 = mot2 = donnees["mot2"]
mot3 = donnees["mot3"]

if len(mot2) != len(mot3):
    resultat_ex15 = {"identiques": False, "position": -1}
else:
    diff_trouve = False
    pos_diff = -1
    for i in range(len(mot2)):
        if not diff_trouve and mot2[i] != mot3[i]:
            diff_trouve = True
            pos_diff = i
    if diff_trouve:
        resultat_ex15 = {"identiques": False, "position": pos_diff}
    else:
